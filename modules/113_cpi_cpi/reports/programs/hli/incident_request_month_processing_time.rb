class Hli::IncidentRequestMonthProcessingTime< Irm::ReportManager::ReportBase
  def data(params={})
    ...
    priorities = Icm::PriorityCode.
        ...
        enabled.order("weight_values asc")

    statis = Icm::IncidentRequest.
        #select("date_format(#{Icm::IncidentRequest.table_name}.submitted_date, '%Y-%m') submitted_month").
        select("#{Icm::IncidentRequest.table_name}.priority_id priority_id, date_format(#{Icm::IncidentRequest.table_name}.last_response_date, '%Y-%m') created_date").
        joins(",#{Icm::IncidentWorkload.table_name} iw").enabled.
        where("#{Icm::IncidentRequest.table_name}.hotline = ?", Irm::Constant::SYS_YES).
    ...
    statis = statis.
        where("date_format(#{Icm::IncidentRequest.table_name}.last_response_date, '%Y-%m-%d') < ?", Date.strptime("#{params[:end][:year]}-#{params[:end][:month]}", '%Y-%m') + 1.month).
        where("date_format(#{Icm::IncidentRequest.table_name}.last_response_date, '%Y-%m-%d') >= ?", Date.strptime("#{params[:start][:year]}-#{params[:start][:month]}", '%Y-%m'))

    statis = statis.
        select("SUM(iw.real_processing_time) duration").
    ...
    statis.each do |s|
      ...
      data[0] = s[:created_date]
      data[column_size - 1] = 0.0
      n = 1
      existed = datas.find{|i| i[0] == s[:created_date]}
