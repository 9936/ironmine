class UploadIssueForGoe2 < ActiveRecord::Migration
  def up
    ...
    issue_list.each do |r|
      ...
      category_id = Icm::IncidentCategory.
          ...
          where("EXISTS (SELECT * FROM icm_incident_category_systems ics WHERE ics.incident_category_id = #{Icm::IncidentCategory.table_name}.id AND ics.external_system_id = ?)", external_system_id).
          where("ict.name = ?", r[4]).
      ...
      subcategory = Icm::IncidentSubCategory.
                ...
                where("ict.name = ?", r[5]).
                where("#{Icm::IncidentSubCategory.table_name}.incident_category_id = ?", category_id).
      ...
      r25 = r[25]
      r25 = r[25].gsub("'", /\\'/.source)
      r26 = r[26]
      r26 = r[26].gsub("'", /\\'/.source)
      r27 = r[27]
      r27 = r[27].gsub("'", /\\'/.source)
      r46 = r[46]
      r46 = r[46].gsub("'", /\\'/.source)
      ...
      execute(%Q(INSERT INTO tmp_icm_incident_requests (request_type_code, report_source_code, id, request_number, title, summary,
                ...
                VALUES ('REQUESTED_TO_PERFORM', 'CUSTOMER_SUBMIT', '#{request_id}','#{request_number}', '#{title}','#{content}',
                        ...
                        '#{r[48]}', '#{r[23]}', '#{r[24]}', '#{r25}', '#{r26}', '#{r[28]}', '#{r[29]}', '#{r27}', '#{r[30]}', '#{r[31]}',
                        '#{r[32]}', '#{r[33]}', '#{r[34]}', '#{r[35]}', '#{r[36]}', '#{r[37]}', '#{r[38]}', '#{r[39]}', '#{r[40]}', '#{r[41]}',
                        '#{r[42]}', '#{r[43]}', '#{r[44]}', '#{r[45]}', '#{r46}', '#{r[47]}')
                ))
