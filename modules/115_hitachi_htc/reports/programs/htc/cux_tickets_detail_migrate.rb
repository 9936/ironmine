class Htc::CuxTicketsDetailMigrate < Irm::ReportManager::ReportBase
  def data(params={})
    params||={}
    ...
    statis.each do |s|
      ...
      aph = Irm::WfStepInstance.
          ...
          joins(",irm_wf_process_instances wpi").
          joins(",irm_people ip").
          where("#{Irm::WfStepInstance.table_name}.approval_status_code = ?", "APPROVED").
          where("ip.id = #{Irm::WfStepInstance.table_name}.actual_approver_id").
          where("wpi.bo_id = ?", s[:id]).
          where("wpi.id = #{Irm::WfStepInstance.table_name}.process_instance_id")
      data[21] = ""
      data[22] = ""
      if aph and aph.size > 0
      aph = aph.first
      data[21] = aph[:end_at]
      data[22] = aph[:approved_by]
      end
