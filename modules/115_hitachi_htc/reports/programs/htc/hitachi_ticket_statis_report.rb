class Htc::HitachiTicketStatisReport < Irm::ReportManager::ReportBase
  def data(params={})
    ...
    people = Irm::Person.
        where("EXISTS (SELECT * FROM #{Irm::GroupMember.table_name} gm WHERE gm.person_id = #{Irm::Person.table_name}.id AND gm.group_id IN (?)", []).
    ...
    on_hand_open = Icm::IncidentRequeset.where("incident_status_id IN (?)", open_status)

    deposit = Icm::IncidentRequeset.
        where("incident_status_id IN (?)", open_status).
        where("last_response_date < ?", Time.now - 3.days)

    resolved = Icm::IncidentRequeset.
            where("incident_status_id IN (?)", resolved_status).
            where("last_response_date < ?", Time.now - 3.days)


    hour_maintainance = Icm::IncidentRequest.
        where("incident_status_code IN (?) AND (cux_resolve_hours IS NULL OR (attribute1 = ? AND ()))", close_status, "21: System Bug")
