  ...
  map.controller "skm/api_entry_headers", {
      ...
      :get_data => {
          ...
          :params => [
              ...
              {:name => :start, :classify => [:input], :type => "Number", :default_value => 0, :description => "用于分页，从第几条记录开始"},
              {:name => :limit, :classify => [:input], :type => "Number", :default_value => 10, :description => "每页显示记录数"},
      ...
      :show => {
          ...
          :description => "查看知识接口",
          :params => [
              {:name => :id, :classify => [:input,:output], :type => "String", :description => "知识文章唯一ID"},
