...
class InitDemLookup < ActiveRecord::Migration
  def up
    ...
    lt.lookup_types_tls.build(:lookup_type_id=> lt.id,:meaning=>'Project',:description=>'Project',:language=>'zh',:status_code=>'ENABLED',:source_lang=>'en')
    lt.lookup_types_tls.build(:lookup_type_id=> lt.id,:meaning=>'Project',:description=>'Project',:language=>'en',:status_code=>'ENABLED',:source_lang=>'en')
    lt.save
