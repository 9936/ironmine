# -*- coding:utf-8 -*-
class Ysq::UncloseLastReply < Irm::ReportManager::ReportBase
  def data(params={})
    params||={}
    ...
    statis = Icm::IncidentRequest.
        ...
        WHERE
            LENGTH(ij1.message_body) > 0 AND ij1.message_body <> 'System Auto Assign' AND ij1.message_body <> '割当ダッシュボード別に割当' AND ij1.message_body <> '分配工作台分派' AND ij1.message_body <> 'Assigned by Assign DashBoard'
        ...
        WHERE
            ipj.id = ij2.replied_by
                 AND LENGTH(ij2.message_body) > 0 AND ij2.message_body <> 'System Auto Assign' AND ij2.message_body <> '割当ダッシュボード別に割当' AND ij2.message_body <> '分配工作台分派' AND ij2.message_body <> 'Assigned by Assign DashBoard'
        ...
        WHERE
            LENGTH(ij3.message_body) > 0 AND ij3.message_body <> 'System Auto Assign' AND ij3.message_body <> '割当ダッシュボード別に割当' AND ij3.message_body <> '分配工作台分派' AND ij3.message_body <> 'Assigned by Assign DashBoard'
