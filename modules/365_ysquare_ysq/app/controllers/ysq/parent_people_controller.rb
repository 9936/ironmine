class Ysq::ParentPeopleController < ApplicationController
  def delete_from_parent
    @parent_person = Ysq::ParentPerson.new(params[:yan_parent_person])

    respond_to do |format|
      if(!params[:temp_id_string].blank?)
        params[:temp_id_string].split(",").delete_if{|i| i.blank?}.each do |id|
          esp = Ysq::ParentPerson.where(:parent_person_id => id).first
          esp.destroy
        end
      end
  ...
  def new_from_person
    ...
    @parent_person = Ysq::ParentPerson.new(:person_id=>params[:id])
    ...
    respond_to do |format|
      if true
        @parent_person.status_code.split(",").delete_if{|i| i.blank?}.each do |id|
          Ysq::ParentPerson.create(:person_id=>params[:id],:parent_person_id=>id)
  ...
  def get_owned_parents_data
    parent_scope = Irm::Person.select("full_name, email_address, #{Irm::Person.table_name}.id").with_organization(I18n.locale).
        joins(",#{Ysq::ParentPerson.table_name} pp").
