class Isp::ProgramTrigger < ActiveRecord::Base
  ...
  has_many :program_schedules,:dependent => :destroy, :foreign_key => :program_trigger_id
  has_many :program_receivers,:dependent => :destroy, :foreign_key => :program_trigger_id
  belongs_to :program
  ...
  def create_receiver_from_str
    ...
    exists_values = Isp::ProgramReceiver.where(:program_trigger_id=>self.id)
    ...
    str_values.each do |value_str|
      ...
      self.program_receivers.create(:receiver_type=>value[0],:receiver_id=>value[1])
    end
  end
  ...
  def get_receiver_str
    ...
    @get_receiver_str||= Isp::ProgramReceiver.where(:program_trigger_id=>self.id).collect{|value| "#{value.receiver_type}##{value.receiver_id}"}.join(",")
  end
  ...
  private
    ...
    # 添理过时的schedule
    def clean_schedule
      Isp::ProgramSchedule.where(:program_trigger_id => self.id, :run_status=>"PENDING").delete_all
    end
