class Fm::Attachment < ActiveRecord::Base
  set_table_name :fm_attachments
  ...
  #Fm::Attachment.accessible('000100012i8IyyjJaqMaJ6')
  scope :accessible, lambda { |person_id|
    select("#{table_name}.*,af.name as folder_name, p.full_name, av.id version_id, av.data_file_name data_file_name, av.download_count download_count, av.data_content_type data_content_type, av.data_file_size/1024 data_file_size, av.data_updated_at data_updated_at").
        joins("LEFT JOIN #{Fm::AttachmentMember.table_name} iam ON (#{table_name}.id=iam.attachment_id AND #{table_name}.created_by != '#{person_id}')").
        joins("LEFT JOIN #{Irm::Person.table_name} p ON #{table_name}.created_by = p.id").
        joins("LEFT JOIN #{Fm::AttachmentVersion.table_name} av ON(av.attachment_id=#{table_name}.id AND av.id = #{table_name}.latest_version_id)").
        #Added by Lei.Peng
        joins("INNER JOIN #{Fm::AttachmentFolder.table_name} af ON(af.id=#{table_name}.folder_id)").
        #added by Lei.Peng
  ...
  scope :list_all, lambda {
    select("#{table_name}.*, p.full_name, av.id version_id, av.category_id category_id, av.data_file_name data_file_name, av.download_count download_count, av.data_content_type data_content_type, av.data_file_size/1024 data_file_size, av.data_updated_at data_updated_at, fvt.meaning category_name").
        ...
        joins(", #{Fm::AttachmentVersion.table_name} av").
  ...
  def create_member_from_str(member_str)
    ...
    exists_values = Fm::AttachmentMember.where(:attachment_id => self.id)
    ...
    str_values.each do |value_str|
      ...
      Fm::AttachmentMember.create(:member_type => value[0], :member_id => value[1], :attachment_id => self.id)
    end
  end
  ...
  def get_member_str
    ...
    @get_member_str||= Fm::AttachmentMember.where(:attachment_id => self.id).collect { |value| "#{value.member_type}##{value.member_id}" }.join(",")
  end
