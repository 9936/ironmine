class Icm::IncidentAmountMonthByCategory < Irm::ReportManager::ReportBase
  def data(params={})
     params||={}
     #获取基本数据
     month_data= Icm::IncidentRequest.select("MONTH(submitted_date)  months, CONCAT(incident_category_id,',',incident_sub_category_id) category,count(1)  amount").
     ...
     #去除重复的和空的和仅包含逗号的类别
     my_cates=my_cates.uniq.delete_if {|x| x==nil or x==','}
     #生成列标题
     cate_headers=[]
  ...
  def to_xls(params)
    columns = [{:key=>0.to_s.to_sym, :label=>I18n.t(:label_report_icm_incident_category)}]
    result = data(params)
    result[:cate_headers].each_with_index do |sh,index|
      columns << {:key=> (index+1).to_s.to_sym,:label=>sh}
    end
    excel_data = []
    result[:datas].each do |data|
