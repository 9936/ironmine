class Icm::IncidentRequestsController < ApplicationController
  ...
  def update_assign_me
    params[:incident_request_ids].split(",").each do |icid|
      incident_request = Icm::IncidentRequest.find(icid)
      timestamp = Time.now
      if Time.now - incident_request.created_at < 1.minutes
        timestamp = timestamp + 1.minutes
      end
      # Delayed::Job.enqueue(Icm::Jobs::AssignMeJob.new(icid, Irm::Person.current.id), 0,timestamp)
      Irm::Person.current = Irm::Person.unscoped.find(Irm::Person.current.id)
      irq = Icm::IncidentRequest.find(icid)
...
#    @file = Irm::Attachment.where(:latest_version_id => params[:att_id]).first
  ...
  def remove_attachment
    ...
    if source.class.name.eql?("Icm::IncidentRequest")
      ...
      source.class.name.eql?("Icm::IncidentJournal")
