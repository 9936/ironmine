class Icm::IncidentRequestsController < ApplicationController
  ...
  def update
    ...
    respond_to do |format|
      flag = true
      flag, now = validate_files(@incident_request) if params[:files].present?
      incident_request_old = @incident_request
      if !flag
        if now.is_a?(Integer)
          flash[:error] = I18n.t(:error_file_upload_limit, :m => Irm::SystemParametersManager.upload_file_limit.to_s, :n => now.to_s)
        else
  ...
  def update_assign_me
    ...
    incident_requests.each do |irq|
      ...
      request_attributes.merge!({:incident_status_id=>Icm::IncidentStatus.transform(irq.incident_status_id,"ASSIGN_TO_ME")})
      ...
      irq.incident_status_id = Icm::IncidentStatus.transform(irq.incident_status_id, incident_journal.reply_type)
