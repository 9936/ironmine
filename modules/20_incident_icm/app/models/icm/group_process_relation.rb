class Icm::GroupProcessRelation < ActiveRecord::Base
  ...
  scope :with_group_from_name, lambda{|language|
    joins("JOIN #{Icm::SupportGroup.multilingual_view_name} sgv0 ON sgv0.id = #{table_name}.support_group_from AND sgv0.language ='#{language}'")
    .select("sgv0.name support_from_name")
  }
