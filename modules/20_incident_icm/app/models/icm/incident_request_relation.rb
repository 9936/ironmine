class Icm::IncidentRequestRelation < ActiveRecord::Base
  ...
  scope :list_all, lambda{|incident_request_id|
    select("ir.id request_id, ir.request_number request_number, ir.title title, #{table_name}.relation_type relation_type,#{table_name}.source_id source_id, #{table_name}.target_id target_id, #{table_name}.id relation_id").
