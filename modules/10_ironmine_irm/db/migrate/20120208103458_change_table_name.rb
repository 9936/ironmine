class ChangeTableName < ActiveRecord::Migration
  def up
    ...
    execute('CREATE OR REPLACE VIEW irm_portal_layouts_vl AS SELECT t.*,tl.id lang_id,tl.name,tl.description,tl.language,tl.source_lang
                         FROM irm_portal_layouts t,irm_portal_layouts_tl tl
                         WHERE t.id = tl.portal_layout_id')
  end

  def down
    drop_table :irm_portal_layouts
    drop_table :irm_portal_layouts_tl
    execute('DROP VIEW irm_portal_layouts_vl')
    #create table
