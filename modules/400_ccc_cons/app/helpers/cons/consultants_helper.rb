module Cons::ConsultantsHelper
  def available_consType
    Cons::ConsType.where("status = ? ","ENABLED").collect{|i|[i[:name],i.id]}
  end

  def available_module
    Cons::Module.where("status = ? ","ENABLED").collect{|i|[i[:name],i.id]}
  end

  def available_status
    Cons::Status.where("status = ? ","ENABLED").collect{|i|[i[:name],i.id]}
  end
  ...
  def available_level
    Cons::Level.where("status = ? ","ENABLED").collect{|i|[i[:name],i.id]}
  end
