				...
				function handle() {
					...
					if ( validator.settings.submitHandler ) {
						if ( validator.submitButton ) {
							...
							hidden = $("<input type='hidden'/>").attr("name", validator.submitButton.name).val( $(validator.submitButton).val() ).after(validator.currentForm);
						}
...
;(function($){
    ...
    $.fn.formValidate = function(setting) {
        ...
        },setting?(setting.messages||{}):{});

        var __setting = $.extend((setting||{}),{
            ignore: "",  //不忽略属性为hidden的表单,对blank_select进行验证
            onfocusout: function(element) { $(element).valid(); },  //转移焦点时对表单进行验证
            rules:__rules,
            messages:__messages,
        ...
        __validate.call(this,__setting);
    };
})(jQuery);
