class Hli::IncidentRequestMonthSummary < Irm::ReportManager::ReportBase
  def data(params={})
    ...
    if params[:hotline].present?
      ...
      last_statis = last_statis.where("#{Icm::IncidentRequest.table_name}.hotline = ?", params[:hotline])
    end
    ...
    last_statis = last_statis.where("date_format(#{Icm::IncidentRequest.table_name}.submitted_date, '%Y-%m') = ?", (Date.strptime("#{year}-#{month}", '%Y-%m') - 1.month).strftime("%Y-%m"))
