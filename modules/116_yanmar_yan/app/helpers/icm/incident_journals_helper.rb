module Icm::IncidentJournalsHelper
  def prepare_request_files(incident_request)
    ...
    if !(allow_to_function?(:view_my_private_reply, sid) && (current_person?(incident_request.submitted_by) ||
  ...
  def sprepare_request_files(incident_request)
    # file belongs to journal
    sid = incident_request.external_system_id
  ...
  def list_all_files
    ...
    @request_files.values.flatten.each do |e|
      html << "<div style='padding-bottom:5px;'><a target='_blank' href='#{e.data.url}' stats="">
              <div class='file-info'><div title='#{e.data.original_filename}(#{e.file_size_kb}KB)' class='file-name'><b class='ellipsis' style='width:150px;'>#{e.data.original_filename}(#{e.file_size_kb}KB)</b></div>
              </div></a></div>"
    end
    raw(html)
  end
  ...
  def slist_journal_files(grouped_files,journal, sid="")
    return if grouped_files[journal.id].nil?||grouped_files[journal.id].size<1
    file_lists = ""
    grouped_files[journal.id].each do |f|
      file_lists << sshow_file(f,true,sid)
    end
    content_tag(:div,file_lists.html_safe,{:class=>"file-list"})
  end
