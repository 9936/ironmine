...
class Boc::BoardSecondsController < ApplicationController
  def index
    ...
    base_query = Icm::IncidentRequest.
    ...
    @sla_list = Icm::IncidentRequest.
        ...
        joins("LEFT OUTER JOIN #{Irm::Organization.view_name} iov ON iov.id = #{Icm::IncidentRequest.table_name}.organization_id AND iov.language = 'zh'").
        select("#{Icm::IncidentRequest.table_name}.request_number request_number,#{Icm::IncidentRequest.table_name}.request_type_code type_code_id,#{Icm::IncidentRequest.table_name}.title title,supporter.full_name supporter_name,#{Icm::IncidentRequest.table_name}.submitted_date submitted_date,iov.name organization_name,sci.service_name service_name").
        enabled.
        ...
        collect{|a|
          ...
          if service_name.eql?("总处理时间超时") && a[:type_code_id].eql?("001b000B2jQm9tAWWwr1jk")
            next
          end
          {
              :request_number=>a[:request_number],
