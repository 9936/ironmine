class Slm::CalendarsController < ApplicationController
  ...
  def synch
    master_calendar=Slm::Calendar.find(params[:id])
    synch_calendars=Slm::Calendar.where("master_code = ? and id <> ?",master_calendar.code,params[:id])
    synch_calendars.each do |synch_calendar|
      #清空原有排班
      synch_calendar.calendar_items.destroy_all
      master_calendar.calendar_items.each do |calendar_item|
      calendar_item=synch_calendar.calendar_items.build({:calendar_year=>calendar_item.calendar_year,
                                           :start_at=>calendar_item.start_at,
                                           :end_at=>calendar_item.end_at,
                                           :calendar_obj=>calendar_item.calendar_obj})
      calendar_item.save
      end
    end if synch_calendars.present?
    respond_to do |format|
        format.html { redirect_to({:action => "show",:notice => t(:label_slm_calendar_master_synch_success,:size=>synch_calendars.size)})}
        format.xml  { head :ok }
    end
  end
end
