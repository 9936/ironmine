module Irm::DuelSelectHelper
  def duel_types(exclude=[])
    ...
    type_classes.collect{|i|
      [Irm::BusinessObject.class_name_to_meaning(i),Irm::BusinessObject.class_name_to_code(i)]
    }
  ...
  def duel_values(exclude=[])
    ...
    if type_classes.include?("business_user")
      label_name =Irm::BusinessObject.class_name_to_meaning("business_user")
      code = Irm::BusinessObject.class_name_to_code("business_user")
      t0 = Temp.new(label_name,code)
      a = []
      a << t0
      values += a.collect{|i|
        ["#{label_name}","#{code}##{i.id}",{:type=>code,:query=>i.name}]
      }
    end
