module Icm::IncidentRequestsHelper
  ...
  def available_support_group(sid = '', object_target = nil)
    if sid.present?
      all_groups = Icm::SupportGroup.enabled.oncall.with_group(I18n.locale).with_system(sid).select_all.order("name asc")
      if object_target.present?
        ...
        if group_process_id.present?
          ...
          all_groups = all_groups.with_group_process(current_support_id, group_process_id).order("name asc")
        end
      end
    else
      all_groups = Icm::SupportGroup.enabled.oncall.with_group(I18n.locale).select_all.order("name asc")
    end
  ...
  def list_ir_existed_entry_headers(incident_request)
    html = ""
    entryHeaderRelations = Ccc::IncidentRequestEntryHeaderRelation.where("source_id = ?", incident_request.id)
    if entryHeaderRelations && entryHeaderRelations.any?
      entryHeaderRelations.each do |a|
        delete_content = (icon_link_delete({:controller => "icm/incident_requests",
                                :action => "remove_entry_header_relation",
        ...
        html << content_tag(:tr,
                       content_tag(:td,
                                   raw(delete_content) +
                                       content_tag(:div,
                                       link_to(Skm::EntryHeader.find(a.target_id).entry_title,
                                       {},
                                       {:href => "javascript:void(0);",:class => "request_info ellipsis",:style => "width:150px;"}),
