...
class Irm::Person < ActiveRecord::Base
  ...
  attr_accessor :old_password,:password, :password_confirmation,:template_flag,:people_nos,:module
  ...
  validates_presence_of :login_name,:first_name,:email_address, :time_zone,:bussiness_phone
...
#  validates_attachment_size :avatar, :less_than => Irm::SystemParametersManager.upload_file_limit.kilobytes
  ...
  # 查询出客户
  # scope :with_people_nos,lambda{
  # #     select("#{table_name}.people_no,#{Irm::Person.name_to_sql(nil,table_name,"person_name")}")
  # select("#{Irm::Person.table_name}.people_no people_nos")
  # }
