# -*- coding: utf-8 -*-
class IronmineAnotherOpuData < ActiveRecord::Migration
  def self.up
    opu = Irm::OperationUnit.create(:name=>"汉得运维中心Lite",
                              :short_name=>"Hand Lite",
                              :description=>"汉得运维中心Lite",
                              :default_language_code=>"zh",
                              :default_time_zone_code=>"GMT_P0800_6")

    enterprise = Irm::License.where(:code=>'ENTERPRISE').first

    opu.update_attributes(:license_id=>enterprise.id,:not_auto_mult=>true)

    Irm::OperationUnit.current = opu
