module Csi::SurveyResultsHelper
  def survey_statistics(survey_id)
    ...
    option_counts.each do |option_count|
      grouped_option_counts[option_count[:survey_subject_option_id]] = {:survey_subject_option_id=>option_count[:survey_subject_option_id],:option_count=>option_count[:option_count],:percent=>response_count==0 ? 0 : option_count[:option_count].fdiv(response_count).round(3)*100}
    end
