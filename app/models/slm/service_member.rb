class Slm::ServiceMember < ActiveRecord::Base
  ...
  scope :query_wrap_info,lambda{|language| select("v1.meaning status_meaning, CONCAT(ip1.last_name,ip1.first_name) service_person_name," +
                                                  "CONCAT(ip2.last_name,ip2.first_name) service_contract_name," +
                                                  "iov.name service_organization_name, #{table_name}.*" ).
                                                   joins("left outer join irm_organizations_vl iov on #{table_name}.service_organization_id=iov.id AND iov.language = '#{I18n.locale}'").
